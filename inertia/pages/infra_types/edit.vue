...
const infraTypeData: any = computed(() => usePage().props?.infraTypeData)
...
const onSubmit = () => {
  ...
  })).put(`/infra-types/${infraTypeData.value.id}`, {
    preserveScroll: true,
    onStart: visit => {
...
onMounted(() => {
  ...
  console.log(infraTypeData.value.attrs)
  console.log(infraTypeData.value.attachments)
  if (infraTypeData.value) {
    form.name = infraTypeData.value.name
    form.code = infraTypeData.value.code
    form.parentId = infraTypeData.value.parentId && infraTypeData.value.parentId > 0 ? infraTypeData.value.parentId : null!
    form.options = infraTypeData.value.options
    form.description = infraTypeData.value.description
    // if (infraTypeData.value.attrs && Array.isArray(infraTypeData.value.attrs)) {
    //   console.log('infraTypeData.value.attrs')
    //   console.log(infraTypeData.value.attrs)
    //   // let attrsData = Array<formAttr>()
    //   // for (let attrData in infraTypeData.value.attrs) {
    //   //   let itemAttr: formAttr = infraTypeData.value.attrs[attrData]
    //   //   attrsData.push(itemAttr)
    //   // }
    //   // form.attrs = attrsData
    //   form.attrs = infraTypeData.value.attrs
    // }

    let attrs = []
    if (infraTypeData.value.attrs && Array.isArray(infraTypeData.value.attrs)) {
      for (let k in infraTypeData.value.attrs) {
        let attr = infraTypeData.value.attrs[k]
        var cloneAttr = { ...attr }
    ...
    <div class="form-outer">
      ...
      <div class="form-body">
        ...
        <div class="form-fieldset">
          ...
          <div class="columns is-multiline">
            <div class="column is-12" v-if="infraTypeData.attachments && infraTypeData.attachments.length">
              <VTags addons v-for="(attachment) in infraTypeData.attachments">
                <VTag :label="attachment.attachment.name" color="primary" />
                ...
                <div class="columns is-multiline">
                  ...
                  <div class="column is-12" v-if="['select'].includes(form.attrs[index].type)">
                    <VCardAdvanced class="mb-4" >
                      ...
                      <template #content>
                        <div v-if="form.attrs[index].options.data && form.attrs[index].options.data.length">
                          <Sortable
